...
async function addDocument(filePath, progressCallback) {
    ...
    for (let i = 0; i < chunks.length; i++) {
        ...
        db.run('INSERT INTO vectors (text, vector) VALUES (?, ?)', [chunk, vector]);
...
async function findSimilar(query, topK = 3) {
    ...
    return new Promise((resolve, reject) => {
        db.all('SELECT text, vector FROM vectors', [], (err, rows) => {
            if (err) {
                reject(err);
            } else {
                const similarities = rows.map(row => {
                    const dbVector = JSON.parse(row.vector);
                    return {
                        text: row.text,
                ...
                resolve(sorted.slice(0, topK).map(r => r.text));
            }
        });
    });
}
